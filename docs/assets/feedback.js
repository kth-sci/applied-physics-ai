 ...
 * Adds a thumbs up/down widget to every page.
 * Thumbs down opens a comment field.
 * Feedback is stored as artifacts in the Hypha kth-sci collection.
 */
(function() {
  const HYPHA = 'https://hypha.aicell.io';
  const COLLECTION_ID = 'kth-sci/aphys-ai-gallery';
  const page = location.pathname.split('/').pop() || 'index.html';

  // Don't add to gallery detail views (hash routes)
  ...
  widget.style.cssText = 'max-width:40rem;margin:3rem auto 0;padding:1.5rem;text-align:center;';
  widget.innerHTML = `
    <div style="border:1px solid #e2e8f0;border-radius:12px;padding:1.5rem;background:#fff;">
      ...
      <div id="fb-buttons" style="display:flex;gap:0.75rem;justify-content:center">
        <button id="fb-up" style="display:inline-flex;align-items:center;gap:0.4rem;padding:0.5rem 1.25rem;border:1px solid #e2e8f0;border-radius:8px;background:#fff;font-family:inherit;font-size:0.875rem;cursor:pointer;transition:all 0.2s;color:#475569"
        ...
        <button id="fb-down" style="display:inline-flex;align-items:center;gap:0.4rem;padding:0.5rem 1.25rem;border:1px solid #e2e8f0;border-radius:8px;background:#fff;font-family:inherit;font-size:0.875rem;cursor:pointer;transition:all 0.2s;color:#475569"
      ...
      <div id="fb-comment" style="display:none;margin-top:1rem">
        <textarea id="fb-text" rows="3" placeholder="What could be improved? Your feedback helps us make this better..."
          style="width:100%;padding:0.6rem 0.75rem;border:1px solid #e2e8f0;border-radius:8px;font-family:inherit;font-size:0.875rem;resize:vertical;outline:none;transition:border 0.2s"
          onfocus="this.style.borderColor='#6366f1'" onblur="this.style.borderColor='#e2e8f0'"></textarea>
        <button id="fb-send" style="margin-top:0.5rem;background:#6366f1;color:#fff;font-weight:600;padding:0.5rem 1.25rem;border-radius:8px;border:none;cursor:pointer;font-size:0.875rem;font-family:inherit">
          Send Feedback
        </button>
      </div>
      <div id="fb-thanks" style="display:none;color:#059669;font-weight:500;font-size:0.875rem;padding:0.5rem 0">
  ...
  async function submitFeedback(vote, comment) {
    ...
    try {
      await fetch(HYPHA + '/public/services/artifact-manager/create', {
        ...
        body: JSON.stringify({
          alias: 'feedback-' + page.replace('.html','') + '-' + Date.now().toString(36),
          parent_id: COLLECTION_ID,
          manifest: {
            name: 'Page Feedback: ' + page,
            type: 'feedback',
            page: page,
            vote: vote,
            comment: comment || '',
    ...
    } catch(e) { /* silent fail — feedback is best-effort */ }
  }
  ...
  document.getElementById('fb-down').addEventListener('click', () => {
    ...
    document.getElementById('fb-down').style.borderColor = '#ef4444';
    document.getElementById('fb-down').style.background = '#fef2f2';
  });

  document.getElementById('fb-send').addEventListener('click', () => {
    const text = document.getElementById('fb-text').value.trim();
    submitFeedback('down', text);
  });
})();
